...
async def invite_auto(bot=get_bot(), ev={}):
    ...
    for item in join_request_list["list"]:
        ...
        if data_save['invite_list'].get(str(item['viewer_id'])):
            ...
            if time.time()-int(data_save['invite_list'][str(item['viewer_id'])]["create_time"])<10*60 or data_save['invite_list'][str(item['viewer_id'])]["old"]:
...
async def invite(uid:str)->str:
    ...
    #     is_accept=True
    # else:
    user_info=await client.callapi("/profile/get_profile", {"target_viewer_id": int(uid), "viewer_id": client.viewer_id})
    # print(user_info)
    ...
    if is_accept:
        ...
        if not is_request:
            ...
            if not int(uid) in new_invite_user_list:
                clan_invite_data = {"invited_viewer_id": int(uid), "invite_message": "猫猫邀请你加入哦", "viewer_id": client.viewer_id}
...
async def invite_check():
    ...
    for item in white_list:
        while client.shouldLogin:
            await client.login()
        user_info=await client.callapi("/profile/get_profile", {"target_viewer_id": item, "viewer_id": client.viewer_id})
        if user_info["clan_name"] != "":
