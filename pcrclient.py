...
class pcrclient:
    ...
    async def callapi(self, apiurl: str, request: dict, crypted: bool = True, noerr: bool = False):
        ...
        try:
            if self.viewer_id is not None:
                request["viewer_id"] = (
                    b64encode(pcrclient.encrypt(str(self.viewer_id), key)) if crypted else str(self.viewer_id)
                )
    ...
    async def login(self):
        ...
        gamestart = await self.callapi(
            "/check/game_start", {"apptype": 0, "campaign_data": "", "campaign_user": randint(0, 99999)}
        )
        ...
        await self.callapi("/home/index", {"message_id": 1, "tips_id_list": [], "is_first": 1, "gold_history": 0})
